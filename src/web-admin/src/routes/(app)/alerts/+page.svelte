<script lang="ts">
	...
	async function refreshWithAI() {
		...
		try {
			...
			if (response.success && response.data) {
				const { newAlertsCount, resolvedAlertsCount, unchangedAlertsCount } = response.data;

				// 結果を表示
				console.log('[refreshWithAI] 更新完了:', response.data);

				if (newAlertsCount > 0 || resolvedAlertsCount > 0) {
					// 何か変更があった場合
					alert(`✅ アラート更新完了\n新規: ${newAlertsCount}件\n解決: ${resolvedAlertsCount}件`);
				} else {
					// 変更がない場合
					alert('変更はありませんでした。すべてのアラートは最新です。');
				}
				...
				{#if selectedAlert.relatedTaskId}
					<div class="detail-section">
						...
						<button
							type="button"
							class="btn-task-link"
							on:click={() => goToTask(selectedAlert!.relatedTaskId!)}
						>
