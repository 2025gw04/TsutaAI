<script lang="ts">
	...
	function calculateStats(taskList: WbsTask[]) {
		...
		stats.completedTasks = flatTasks.filter((t) => t.status === 'completed').length;
		stats.inProgressTasks = flatTasks.filter((t) => t.status === 'in-progress').length;
		stats.notStartedTasks = flatTasks.filter((t) => t.status === 'not-started' || !t.status).length;
		stats.blockedTasks = flatTasks.filter((t) => t.status === 'blocked').length;

		// 期限超過タスク
		const today = new Date().toISOString().split('T')[0];
		stats.overdueTasks = flatTasks.filter(
			(t) => t.endDate && t.endDate < today && t.status !== 'completed'
		).length;
		...
		stats.statusDistribution = {
			'not-started': flatTasks.filter((t) => !t.status || t.status === 'not-started').length,
			planning: flatTasks.filter((t) => t.status === 'planning').length,
			'in-progress': flatTasks.filter((t) => t.status === 'in-progress').length,
	...
	{:else}
		<header class="dashboard-header">
			<div>
				...
				<p>{projectData?.project_name || 'プロジェクト'}</p>
			</div>
