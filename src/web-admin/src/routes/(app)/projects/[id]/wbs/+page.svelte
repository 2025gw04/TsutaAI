<script lang="ts">
	...
	function normalizeTaskFromApi(apiTask: any): WbsTask {
		// assignedToからユーザー名を検索
		let assigneeName = '';
		if (apiTask.assignedTo) {
			const assignedUser = users.find((u) => u.id === apiTask.assignedTo);
		...
		return {
			...
			effortDays: apiTask.estimatedHours ? apiTask.estimatedHours / 8 : 1,
			...
			dependencies: apiTask.dependencies ? JSON.parse(apiTask.dependencies) : [],
			deliverable: apiTask.deliverable || '',
			parentTaskId: apiTask.parentTaskId ? String(apiTask.parentTaskId) : undefined,
			children: []
		};
	}
