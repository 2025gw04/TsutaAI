<script lang="ts">
	...
	async function viewRequestDetails(requestId: number) {
		...
		try {
			...
			helperSuggestions = suggestionsRes.data || [];
		...
		>
			<div class="modal-content modal-large">
				...
				<div class="modal-footer">
					...
					<button class="btn-primary" on:click={createHelpRequest} disabled={loading}>
						{loading ? '作成中...' : 'リクエスト作成'}
					</button>
				</div>
			</div>
			...
			<div class="modal-content modal-padded" on:click|stopPropagation>
				...
				<div class="modal-actions">
					<button class="btn-secondary" on:click={() => (showAssignModal = false)}
						>キャンセル</button
					>
					<button class="btn-primary" on:click={assignHelper} disabled={!assignHelperId || loading}>
						{loading ? '割り当て中...' : '割り当て'}
					</button>
				</div>
			</div>
	...
	{#if showResolveModal}
		...
		<div class="modal-overlay" on:click={() => (showResolveModal = false)}>
			<div class="modal-content modal-padded" on:click|stopPropagation>
				...
				<div class="modal-actions">
					<button class="btn-secondary" on:click={() => (showResolveModal = false)}
						>キャンセル</button
					>
					<button
						class="btn-primary"
						on:click={resolveRequest}
						disabled={!resolutionNote || loading}
					>
						{loading ? '解決中...' : '解決'}
					</button>
				</div>
