<script lang="ts">
	...
	async function createSprint() {
		...
		try {
			await apiClient.createSprint({
				...
				sprintName: createForm.sprintName,
				goalDescription: createForm.sprintGoal,
				startDate: createForm.startDate,
				endDate: createForm.endDate,
				targetStoryPoints: createForm.targetStoryPoints ?? undefined,
				createdBy: $auth?.id || 1
			});
	...
	async function selectSprint(sprintId: number) {
		...
		try {
			const [sprintRes, statsRes, progressRes, latestRes] = await Promise.all([
				apiClient.fetchSprint(sprintId),
				apiClient.fetchSprintStats(sprintId),
				apiClient.fetchSprintProgress(sprintId, 30),
				apiClient.fetchLatestSprintProgress(sprintId)
			]);

			selectedSprint = sprintRes.data;
			sprintStats = statsRes.data;
			sprintProgress = progressRes.data || [];
			latestProgress = latestRes.data;
			activeTab = 'overview';
			...
			if (selectedSprint.status !== 'planning') {
				try {
					...
					memberPerformance = perfRes.data || [];
				} catch (err) {
	...
	async function startSprint(sprintId: number) {
		...
		error = '';
		try {
			await apiClient.startSprint(sprintId);
			await loadSprints();
	...
	async function completeSprint(sprintId: number) {
		...
		error = '';
		try {
			await apiClient.completeSprint(sprintId);
			await loadSprints();
			if (selectedSprint?.id === sprintId) {
				await selectSprint(sprintId);
			}
	...
	async function recordProgress() {
		...
		error = '';
		try {
			await apiClient.recordSprintProgress(selectedSprint.id);
			const [progressRes, latestRes] = await Promise.all([
				apiClient.fetchSprintProgress(selectedSprint.id, 30),
				apiClient.fetchLatestSprintProgress(selectedSprint.id)
			]);
			sprintProgress = progressRes.data || [];
			latestProgress = latestRes.data;
		} catch (err: any) {
	...
	async function recordPerformance() {
		...
		error = '';
		try {
			await apiClient.recordSprintPerformance(selectedSprint.id);
			const perfRes = await apiClient.fetchSprintPerformance(selectedSprint.id);
			memberPerformance = perfRes.data || [];
		} catch (err: any) {
	...
	async function analyzeGoal() {
		...
		error = '';
		try {
			const res = await apiClient.analyzeSprintGoal(selectedSprint.id);
			aiAnalysis = res.data;
			activeTab = 'analysis';
		} catch (err: any) {
	...
	function getChartData(sprint: any, progressData: any[]) {
		...
		const getX = (dateStr: string) => {
			...
			return padding + (diff / (totalDays - 1)) * graphWidth;
		};
		...
		for (let i = 0; i < totalDays; i += Math.ceil(totalDays / 10)) {
			...
			xLabels.push({
				x: padding + (i / (totalDays - 1)) * graphWidth,
				y: height - padding + 20,
...
<div class="sprints-page">
		...
		<div class="create-form">
			...
			<div class="form-grid">
				<div class="form-group">
					...
					<select id="projectId" bind:value={createForm.projectId}>
	...
	<div class="filters">
		<div class="filter-group">
			...
			<select id="filterProject" bind:value={filterProject} on:change={loadSprints}>
		...
		{#if selectedSprint}
			<div class="details-panel">
				...
				<div class="action-buttons">
					...
					{/if}
					<button class="btn-secondary" on:click={analyzeGoal} disabled={loading}>
						AI分析実行
					</button>
				</div>
						...
						<div class="progress-section">
							<div class="section-header">
								...
								<button
									...
									disabled={loading || selectedSprint.status !== 'active'}
								>
									進捗記録
								</button>
							</div>
						...
						<div class="performance-section">
							<div class="section-header">
								...
								<button
									...
									disabled={loading || selectedSprint.status === 'planning'}
								>
									パフォーマンス記録
								</button>
							</div>
					...
					{:else if activeTab === 'analysis'}
						<div class="analysis-section">
							<div class="section-header">
								<h4>AI分析結果</h4>
								{#if !aiAnalysis}
									<button class="btn-small" on:click={analyzeGoal} disabled={loading}>
										分析実行
									</button>
								{/if}
							</div>
