				...
				<div class="result-card" style="border-color: {getResultTypeColor(result.result_type)}">
					<div class="card-header" style="background: {getResultTypeColor(result.result_type)}15">
						...
						<span class="confidence">
							信頼度: {(result.confidence_level * 100).toFixed(0)}%
						</span>
					</div>
						...
						<div class="metric-row">
							<div class="metric">
								...
								<div class="metric-content">
									...
									<span class="metric-value">{result.total_effort.toFixed(1)} 人日</span>
								</div>
							</div>
							<div class="metric">
						...
						<div class="metric-row">
							<div class="metric">
								...
								<div class="metric-content">
									...
									<span class="metric-value">{result.team_size.toFixed(1)} 人</span>
								</div>
							</div>
							{#if result.total_cost}
								<div class="metric">
									...
									<div class="metric-content">
										...
										<span class="metric-value">{result.total_cost.toLocaleString()} 円</span>
									</div>
								</div>
							{/if}
						</div>
							...
							<div class="ai-savings-banner">
								...
								<div>
									...
									<div class="savings-value">
										{result.breakdown.aiSavings.toFixed(1)} 人日の工数削減
									</div>
								</div>
							</div>
						{/if}
									...
									{#each result.breakdown.phases as phase}
										<div class="phase-row">
											...
											<div class="phase-metrics">
												<span class="phase-effort">{phase.effort.toFixed(1)}人日</span>
												<span class="phase-duration">{phase.duration}日</span>
												{#if phase.useAi}
													<span class="phase-ai-badge">
														<i class="bi bi-robot"></i>
														AI {(phase.aiEfficiencyRatio * 100).toFixed(0)}%
													</span>
												{/if}
											</div>
										</div>
				...
				<table>
					...
					<tbody>
						<tr>
							<td>総工数</td>
							{#each results as result}
								<td>{result.total_effort.toFixed(1)} 人日</td>
							{/each}
						</tr>
						<tr>
							<td>期間</td>
						...
						<tr>
							<td>平均人数</td>
							{#each results as result}
								<td>{result.team_size.toFixed(1)} 人</td>
							{/each}
						</tr>
					</tbody>
				</table>
