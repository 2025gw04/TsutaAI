<script lang="ts">
	...
	async function generateReport() {
		try {
			...
			if (reportType === 'project' && projectId) {
				...
				// Markdown/HTMLの場合はレスポンス自体が文字列、JSONの場合はresponse.dataを取得
				generatedReport =
					format === 'json' && typeof response === 'object' && response.data
						? response.data
						: response;
	...
	function exportCSV() {
		if (projectId) {
			apiClient.exportProjectReportCSV(projectId);
		}
	}
...
<div class="report-generator">
	...
	<div class="action-buttons">
		...
		{#if reportType === 'project' && projectId}
			<button type="button" class="btn-export" on:click={exportCSV}>
