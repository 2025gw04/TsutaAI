		...
		if (!response.ok) {
			...
			if (contentType && contentType.includes('application/json')) {
				...
				try {
					...
					const messagePart = errorData.message || errorData.error || '';
					const detailsPart = errorData.details || '';
					errorMessage = messagePart || detailsPart || errorMessage;
					if (messagePart && detailsPart && detailsPart !== messagePart) {
...
export const apiClient = {
	...
	fetchNotifications: () => request<{ success: boolean; data: any[] }>('/notifications'),
	fetchUnreadNotifications: () => request<{ success: boolean; data: any[] }>('/notifications/unread'),
	markNotificationAsRead: (id: number) =>
		request<{ success: boolean; data: { updated: boolean } }>(`/notifications/${id}/read`, {
			method: 'PUT'
		}),
	markAllNotificationsAsRead: () =>
		request<{ success: boolean; data: { updatedCount: number } }>('/notifications/read-all', {
			method: 'PUT'
		}),
	...
	saveAlerts: (alerts: any[]) =>
		request<{
			success: boolean;
			data?: { inserted: number; updated: number; unchanged: number };
			message?: string;
		}>('/dashboards/alerts', {
			method: 'POST',
	...
	createMentalHealthLog: (payload: {
		...
		request<{ success: boolean; data: any }>('/mental-health', {
			...
			body: JSON.stringify({
				...
				has_blocker: payload.hasBlocker,
				blocker_details: payload.blockerDetails,
				need_support: payload.needSupport,
				support_details: payload.supportDetails
			})
		}),
	...
	// 互換用エイリアス: 設定一括更新は /settings/bulk を使用
