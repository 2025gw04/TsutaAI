<script lang="ts">
	...
	async function savePhase() {
		...
		try {
			...
			const phasePayload = {
				id: formData.id,
				phaseName: formData.phase_name.trim(),
				phaseOrder: formData.phase_order ?? phases.length + 1,
				effort: formData.effort ?? 0,
				durationDays: formData.duration_days ?? 0,
				teamSize: formData.team_size ?? 1,
				startDate: formData.start_date,
				endDate: formData.end_date,
				dependencies: formData.dependencies,
				useAi: formData.use_ai ?? false,
				aiEfficiencyRatio: formData.ai_efficiency_ratio ?? 0.2,
				aiEfficiencyAuto: formData.ai_efficiency_auto ?? true
			};
	...
	async function handleDeletePhase() {
		if (isSubmitting || !editingPhase || !editingPhase.id) return;
	...
	<div class="modal-overlay" on:click={() => (showAddModal = false)}>
		<div class="modal-content" on:click|stopPropagation>
			...
			<div class="modal-footer">
				{#if editingPhase}
					<button class="btn-danger" on:click={handleDeletePhase} disabled={isSubmitting}>
						<i class="bi bi-trash"></i> 削除
					</button>
				{/if}
				<button class="btn-secondary" on:click={() => (showAddModal = false)} disabled={isSubmitting}>
					キャンセル
				</button>
				<button class="btn-primary" on:click={savePhase} disabled={isSubmitting}> 保存 </button>
			</div>
		</div>
	</div>
{/if}
