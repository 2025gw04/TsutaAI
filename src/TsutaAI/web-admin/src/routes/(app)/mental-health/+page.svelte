<script lang="ts">
	...
	async function loadDashboard(force = false) {
		...
		try {
			...
			teamSummary = isRecord(summaryRes?.data) ? summaryRes.data : null;

			// サポート必要ユーザーはteamSummaryに含まれている
			needSupportUsers = asArray(teamSummary?.alerts?.needSupportUsers);
		} catch (err: any) {
	...
	async function selectUser(user: any) {
		...
		try {
			...
			const [trendRes, activityRes] = await Promise.all([
				apiClient.analyzeMentalHealthTrend(userId, 4),
				apiClient.detectActivityAnomaly(userId)
			]);

			selectedUserTrend = isRecord(trendRes?.data) ? trendRes.data : null;
			selectedUserActivity = isRecord(activityRes?.data) ? activityRes.data : null;
		} catch (err: any) {
	...
	async function runAIAnalysis() {
		if (!selectedUser || typeof selectedUser.userId !== 'number' || isAnalyzing) return;
		...
		try {
			...
			if (result?.success && isRecord(result.analysis)) {
				aiAnalysis = result.analysis;
			} else {
				analysisError =
					(typeof result?.error === 'string' && result.error) || 'AI分析に失敗しました';
			}
		} catch (err: any) {
				...
				<div class="details-grid">
					...
					<div class="detail-card full-width">
						...
						{#await apiClient.fetchMentalHealthLogs(selectedUser.userId, { days: 30 }) then logsRes}
							{@const logs = asArray(logsRes?.data)}
							{#if logs.length > 0}
								<div class="logs-list">
									{#each logs as log}
										<div class="log-item">
					...
					<div class="detail-card full-width">
						...
						{#await apiClient.fetchReports({ userId: selectedUser.userId }) then reportsRes}
							{@const reports = asArray(reportsRes?.data).slice(0, 14).reverse()}
							{#if reports.length > 0}
					...
					<div class="detail-card full-width ai-analysis-card">
						...
						{:else if aiAnalysis}
							<div class="analysis-results">
								...
								<!-- 懸念点 -->
								{#if Array.isArray(aiAnalysis.concerns) && aiAnalysis.concerns.length > 0}
									<div class="analysis-section concerns">
										<h4>⚠️ 懸念点</h4>
										<ul>
											{#each asArray(aiAnalysis.concerns) as concern}
												<li>{concern}</li>
											{/each}
										</ul>
									</div>
								...
								<!-- 良い点 -->
								{#if Array.isArray(aiAnalysis.positive_points) && aiAnalysis.positive_points.length > 0}
									<div class="analysis-section positive">
										<h4>✨ 良い点</h4>
										<ul>
											{#each asArray(aiAnalysis.positive_points) as point}
												<li>{point}</li>
											{/each}
										</ul>
									</div>
								...
								<!-- 推奨アクション -->
								{#if Array.isArray(aiAnalysis.recommended_actions) &&
									aiAnalysis.recommended_actions.length > 0}
									<div class="analysis-section actions">
										<h4>🎯 推奨アクション</h4>
										<ol>
											{#each asArray(aiAnalysis.recommended_actions) as action}
												<li>{action}</li>
											{/each}
										</ol>
									</div>
								...
								<!-- 1on1質問 -->
								{#if Array.isArray(aiAnalysis.follow_up_questions) &&
									aiAnalysis.follow_up_questions.length > 0}
									<div class="analysis-section questions">
										<h4>💬 1on1で確認すべき質問</h4>
										<ul>
											{#each asArray(aiAnalysis.follow_up_questions) as question}
												<li>{question}</li>
											{/each}
										</ul>
									</div>
					...
					{#if selectedUserTrend}
						<div class="detail-card">
							...
							<div class="trend-status">
								<strong>トレンド:</strong>
								{getTrendEmoji(selectedUserTrend.trend || 'stable')}
							</div>
							{#if Array.isArray(selectedUserTrend.weeklyData) && selectedUserTrend.weeklyData.length > 0}
					...
					{#if selectedUserActivity}
						<div class="detail-card">
							...
							{#if selectedUserActivity.hasAnomaly}
								{@const anomalies = asArray(selectedUserActivity.anomalies)}
