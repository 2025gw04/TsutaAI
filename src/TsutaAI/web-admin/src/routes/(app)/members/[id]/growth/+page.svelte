<script lang="ts">
	...
	function validateUserId(value: number): number | null {
		return Number.isInteger(value) && value > 0 ? value : null;
	}

	async function loadGrowthData() {
		const normalizedUserId = validateUserId(userId);
		const normalizedMonths = Number(selectedMonths) || 6;
		const requestKey = `${normalizedUserId ?? 'invalid'}:${normalizedMonths}`;
		...
		if (isLoadingRequest && requestKey === lastRequestKey) {
			return;
		}
		...
		try {
			...
			// 成長レポートを取得
			const reportResponse = await apiClient.fetchGrowthReport(normalizedUserId, {
				months: normalizedMonths,
				includeGoals: true,
				includeContributions: true
			});
			if (requestId !== activeRequestId) return;
			growthReport = reportResponse.data ?? null;

			// スキル成長履歴を取得
			const skillResponse = await apiClient.fetchSkillGrowthHistory(normalizedUserId, {
				months: normalizedMonths
			});
			if (requestId !== activeRequestId) return;
			skillHistory = ensureArray<any>(skillResponse.data);

			// パフォーマンスメトリクスを取得
			const metricsResponse = await apiClient.fetchPerformanceMetrics(normalizedUserId, normalizedMonths);
			if (requestId !== activeRequestId) return;
			performanceMetrics = ensureArray<any>(metricsResponse.data);

			// 貢献記録を取得
			const endDate = new Date().toISOString().split('T')[0];
			const startDate = new Date(Date.now() - normalizedMonths * 30 * 24 * 60 * 60 * 1000)
				.toISOString()
				.split('T')[0];
			const contributionsResponse = await apiClient.fetchContributions(normalizedUserId, {
				startDate,
				endDate
			});
			if (requestId !== activeRequestId) return;
			contributions = ensureArray<any>(contributionsResponse.data);

			// 成長目標を取得
			const goalsResponse = await apiClient.fetchGrowthGoals(normalizedUserId);
			if (requestId !== activeRequestId) return;
			goals = ensureArray<any>(goalsResponse.data);
		} catch (error) {
			if (requestId !== activeRequestId) return;
