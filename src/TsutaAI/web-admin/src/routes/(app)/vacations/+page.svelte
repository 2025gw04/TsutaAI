<script lang="ts">
	...
	let deletingVacationIds: number[] = $state([]);
	...
	function getDefaultUserId() {
		...
		return typeof firstUser?.id === 'number' ? firstUser.id : 0;
	}

	function getDefaultVacationForm() {
		return {
	...
	function resetAddVacationForm() {
		newVacation = getDefaultVacationForm();
	}

	function closeAddModal() {
		showAddModal = false;
		resetAddVacationForm();
	}
	...
	async function loadVacations() {
		try {
			...
			vacations = Array.isArray(response?.data) ? response.data : [];
		} catch (error) {
	...
	async function handleAddVacation() {
		...
		try {
			isAdding = true;
			const userId = Number(newVacation.userId);
			if (!Number.isInteger(userId) || userId <= 0) {
				alert('有効なユーザーを選択してください。');
				return;
			}

			await apiClient.createVacation({
				...newVacation,
				userId
			});
			closeAddModal();
	...
	async function handleDelete(id: number) {
		if (deletingVacationIds.includes(id)) return;
		if (!confirm('この休暇を削除しますか？')) return;

		try {
			deletingVacationIds = [...deletingVacationIds, id];
			await apiClient.deleteVacation(id);
			await loadVacations();
		} catch (error) {
			alert('休暇の削除に失敗しました');
		} finally {
			deletingVacationIds = deletingVacationIds.filter((vacationId) => vacationId !== id);
		}
	}
	...
	async function handleAnalyze() {
		...
		try {
			...
			impactAnalysis = Array.isArray(response?.data) ? response.data : [];
		} catch (error) {
			alert('影響分析に失敗しました');
			impactAnalysis = [];
		} finally {
					...
					<article class="impact-card">
						...
						<div class="stat">
							...
							<ul class="vacation-list">
								{#each Array.isArray(impact.vacations) ? impact.vacations : [] as vacation}
									<li>
						...
						<tr>
							...
							<td data-label="操作">
								<button
									...
									disabled={deletingVacationIds.includes(vacation.id)}
								>
		...
		>
			...
			>
				...
				<div class="modal-form-actions">
					<button type="button" class="btn ghost" onclick={closeAddModal}>
						キャンセル
					</button>
