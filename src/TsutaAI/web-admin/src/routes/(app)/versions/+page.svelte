<script lang="ts">
	...
	let filterTable = '';
	let filterAction = '';
	let loadingVersionsRequest = false;
	const versionActions = new Set<VersionHistory['action']>(['INSERT', 'UPDATE', 'DELETE']);

	const sampleVersions: VersionHistory[] = [
		{
			id: 1,
			tableName: 'projects',
			recordId: 1,
	...
	function normalizeVersionsPayload(payload: unknown): VersionHistory[] {
		if (!Array.isArray(payload)) return [];

		return payload
			.map((item, index): VersionHistory | null => {
				if (!isRecord(item)) return null;

				const action = String(item.action ?? '').toUpperCase() as VersionHistory['action'];
				if (!versionActions.has(action)) return null;

				return {
					id: toNumber(item.id, index + 1),
					tableName: String(item.tableName ?? item.table_name ?? 'unknown'),
					recordId: toNumber(item.recordId ?? item.record_id, 0),
					action,
					changedBy: toNumber(item.changedBy ?? item.changed_by, 0),
					changedByName: String(item.changedByName ?? item.changed_by_name ?? '不明'),
					changedAt:
					...
					oldValues: isRecord(item.oldValues) || Array.isArray(item.oldValues) ? item.oldValues : null,
					newValues: isRecord(item.newValues) || Array.isArray(item.newValues) ? item.newValues : null
				};
			})
			.filter((item): item is VersionHistory => item !== null);
	}
	...
	async function loadVersions() {
		if (loadingVersionsRequest) return;

		loadingVersionsRequest = true;
		try {
			loading = true;
			error = '';
			...
			const response = await apiClient.get('/activity-logs/team/stats?hours=168');
			const normalized = normalizeVersionsPayload(response?.data);
			versions = normalized.length > 0 ? normalized : sampleVersions;
		} catch (e: any) {
			error = e.message || 'バージョン履歴の読み込みに失敗しました。';
			versions = sampleVersions;
		} finally {
			loadingVersionsRequest = false;
			loading = false;
		}
	}
	...
	$: filteredVersions = (Array.isArray(versions) ? versions : []).filter((v) => {
...
<div class="container-fluid py-4">
	<div class="d-flex justify-content-between align-items-center mb-4 desktop-header">
		...
		<button class="btn btn-outline-primary" on:click={loadVersions} disabled={loadingVersionsRequest}>
			{loadingVersionsRequest ? '更新中...' : '再読み込み'}
		</button>
	</div>
