...
namespace TsutaAI.Windows
{
    ...
    public partial class ProjectDashboardWindow : Window
    {
        ...
        {
            if (!_selectedProjectId.HasValue || _isLoadingDashboardData)
            {
                return;
            }
            ...
            try
            {
                ...
                await LoadHealthScoreAsync(projectId);
                await LoadBurndownDataAsync(projectId);
                await LoadSprintsAsync(projectId);
        ...
        private async Task LoadHealthScoreAsync(int? projectId = null)
        {
            var targetProjectId = projectId ?? _selectedProjectId;
            ...
            try
            {
                var healthScore = await _apiService.GetLatestHealthScoreAsync(targetProjectId.Value);
                ...
                var history = await _apiService.GetHealthScoreHistoryAsync(targetProjectId.Value, 30);
                await Dispatcher.InvokeAsync(() =>
                {
                    if (history != null)
                    {
        ...
        {
            await RunWithButtonGuardAsync(sender, async () =>
            {
        ...
        private async Task LoadBurndownDataAsync(int? projectId = null)
        {
            var targetProjectId = projectId ?? _selectedProjectId;
            if (!targetProjectId.HasValue)
            {
            ...
            try
            {
                var burndownData = await _apiService.GetBurndownDataAsync(targetProjectId.Value, 30);
        ...
        private async Task LoadSprintsAsync(int? projectId = null)
        {
            var targetProjectId = projectId ?? _selectedProjectId;
            if (!targetProjectId.HasValue)
            {
            ...
            try
            {
                var allSprints = await _apiService.GetSprintsAsync(targetProjectId.Value);
        ...
        private static async Task RunWithButtonGuardAsync(object sender, Func<Task> action)
        {
