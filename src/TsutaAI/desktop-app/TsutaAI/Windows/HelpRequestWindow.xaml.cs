...
namespace TsutaAI.Windows
{
    ...
    public partial class HelpRequestWindow : Window
    {
        ...
        private async void HelpRequestWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                // 自分のタスク一覧を取得
                var tasksList = await _apiService.GetUserTasksAsync(_userId) ?? new List<TaskItem>();
        ...
        private async void BackOrDirectButton_Click(object sender, RoutedEventArgs e)
        {
            if (_isSuggestionView)
            {
                // Go Back to Input
        ...
        private async void GenerateDraftButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (preview == null)
                {
                    throw new InvalidOperationException("AI推敲結果の取得に失敗しました。");
                }
        ...
        {
            if (_isLoadingSuggestions || _isSubmitting)
            {
                return;
            }
            ...
            try
            {
                ...
                BackOrDirectButton.IsEnabled = false;
                ...
                if (preview == null)
                {
                    throw new InvalidOperationException("AI候補検索結果の取得に失敗しました。");
                }
                ...
                // Bind Suggestions
                var suggestions = preview.Suggestions ?? new List<HelperSuggestion>();
                SuggestionsList.ItemsSource = suggestions;
                if (suggestions.Count > 0)
                {
                    var scoreLog = string.Join(", ", suggestions.Select((s, i) =>
            ...
            finally
            {
                ...
                ActionButton.IsEnabled = !_isSubmitting;
                BackOrDirectButton.IsEnabled = !_isSubmitting;
        ...
        {
            if (_isSubmitting || !ValidateInput()) return;
            ...
            try
            {
                ...
                if (CreatedHelpRequest == null)
                {
                    throw new InvalidOperationException("ヘルプリクエストの作成結果が取得できませんでした。");
                }
            ...
            finally
            {
                ...
                BackOrDirectButton.IsEnabled = true;
