...
namespace TsutaAI.Windows
{
    ...
    public partial class SettingsWindow : Window
    {
        ...
        private bool _isTestingVcsConnection;
        ...
        private void SaveSettings_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var settings = _configService.LoadSettings() ?? new ApplicationSetting();
                settings.ApiUrl = ApiUrlTextBox.Text;
                settings.WorkFolder = WorkFolderTextBox.Text;
                settings.MonitorFileChanges = MonitorFileChangesCheckBox.IsChecked ?? true;
                settings.MonitorGit = MonitorGitCheckBox.IsChecked ?? true;
                settings.MonitorMouse = MonitorMouseCheckBox.IsChecked ?? true;
                settings.MonitorKeyboard = MonitorKeyboardCheckBox.IsChecked ?? true;
                settings.MonitorActiveWindow = MonitorActiveWindowCheckBox.IsChecked ?? true;
                settings.AutoSaveIntervalMinutes = GetAutoSaveInterval();
                settings.IdleTimeMinutes = GetIdleTime();
                settings.NotifyTaskStart = NotifyTaskStartCheckBox.IsChecked ?? true;
                settings.NotifyTaskEnd = NotifyTaskEndCheckBox.IsChecked ?? true;
                settings.NotifyBreak = NotifyBreakCheckBox.IsChecked ?? false;
                settings.NotifyAiAlert = NotifyAiAlertCheckBox.IsChecked ?? true;
                settings.NotifySound = NotifySoundCheckBox.IsChecked ?? true;
                settings.Vacations = settings.Vacations ?? new System.Collections.Generic.List<VacationInfo>();
                settings.Vacations.Clear();
        ...
        {
            if (TestConnectionButton != null && !TestConnectionButton.IsEnabled)
            {
                return;
            }
        ...
        {
            var actionButton = sender as System.Windows.Controls.Button ?? AddSkillButton;
            ...
            if (App.CurrentUser == null || App.ApiService == null)
            {
                Alert.Error("API接続またはログイン状態を確認してください。", "エラー");
                return;
            }
        ...
        private async void UpdateProfile_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (App.ApiService == null)
                {
                    Alert.Error("APIサービスが初期化されていません。", "エラー");
                    return;
                }
        ...
        /// VCS接続をテストします。
        /// </summary>
        private async void TestVcsConnection_Click(object sender, RoutedEventArgs e)
        {
            if (_isTestingVcsConnection)
            {
                return;
            }

            _isTestingVcsConnection = true;
            var testButton = sender as System.Windows.Controls.Button ?? TestVcsConnectionButton;
            ...
            try
            {
                ...
                var username = VcsUsernameTextBox.Text;
                var password = VcsPasswordBox.Password;
                ...
                if (vcsType == 0) // Git
                {
                    var isGitRepository = await Task.Run(() => GitService.IsGitRepository(localPath));
                    if (isGitRepository)
                    {
                        var branch = await Task.Run(() =>
                        {
                ...
                {
                    var isSvnRepository = await Task.Run(() => SvnService.IsSvnRepository(localPath));
                    if (isSvnRepository)
                    {
                        var info = await Task.Run(() =>
                        {
                            var svnService = new SvnService(localPath, username, password);
                            return svnService.GetRepositoryInfo();
                        });
