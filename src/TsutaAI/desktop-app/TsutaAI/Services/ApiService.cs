...
namespace TsutaAI.Services
{
    ...
    public class ApiService
    {
        ...
        private static string NormalizeTaskStatusForApi(string status)
        {
        ...
        public async Task SendWorkLogAsync(WorkLog log)
        {
            try
            {
                if (log == null || log.TaskId <= 0)
                {
                    Logger.Info("タスクIDが未設定のため、作業ログ送信をスキップしました。");
                    return;
                }
        ...
        public async Task<bool> SubmitDailyReportAsync(DailyReport report)
        {
            var payload = new
            {
                userId = report.UserId,
                reportDate = report.ReportDate.ToString("yyyy-MM-dd"),
                summary = report.GeneratedSummary,
                generatedSummary = report.GeneratedSummary,
                comment = report.EditedContent,
                satisfactionLevel = report.SatisfactionLevel,
                achievementRate = report.AchievementRate,
                focusLevel = report.FocusLevel,
                difficultyLevel = report.DifficultyLevel,
                learningLevel = report.LearningLevel,
                aiGenerated = report.AiGenerated
            };

            await SendAsync<object>(HttpMethod.Post, "/reports", payload);
            return true;
        }
        ...
        public async Task<bool> UpdateTaskStatusAsync(int taskId, string status, int progress)
        {
            try
            {
                var apiStatus = NormalizeTaskStatusForApi(status);

                // ステータスと進捗率を含めたペイロードを作成
                var payload = new { status = apiStatus, progress };
        ...
        public async Task<bool> UpdateHelpRequestAsync(int requestId, object updateData)
        {
            try
            {
                await SendAsync<object>(new HttpMethod("PATCH"), $"/help-requests/{requestId}", updateData);
                return true;
            }
            catch (Exception ex)
            {
                Logger.Error($"ヘルプリクエスト更新に失敗しました: {ex.Message}");
                return false;
            }
        }
        ...
        public async Task<AiAnalysisResponse> TriggerHourlyActivityAnalysisAsync(int summaryId)
        {
            try
            {
                ...
                var response = await _client.PostAsync("/api/ai/analyze-hourly-activity", content);
        ...
        public async Task<HourlyActivitySubmitResponse> SendHourlyActivitySummaryAsync(HourlyActivitySummary summary)
        {
            try
            {
                ...
                var response = await _client.PostAsync("/api/hourly-activity", content);
        ...
        public async Task<List<MentalHealthLog>> GetMentalHealthLogsAsync(int userId, int days = 30)
        {
            try
            {
                ...
                return await SendAsync<List<MentalHealthLog>>(HttpMethod.Get, url);
            }
            catch (Exception ex)
            {
                Logger.Error($"メンタルヘルスログ取得に失敗しました: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<MentalHealthLog> GetMentalHealthLogByDateAsync(int userId, string date)
        {
            try
            {
                return await SendAsync<MentalHealthLog>(HttpMethod.Get, $"/mental-health/user/{userId}/date/{date}");
            }
            catch (Exception ex)
            {
                Logger.Warn($"特定日のメンタルヘルスログ取得に失敗しました: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<Project> CreateProjectAsync(Project project)
        {
            try
            {
                ...
                return await SendAsync<Project>(HttpMethod.Post, "/projects", payload);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> UpdateProjectAsync(int projectId, Project project)
        {
            try
            {
                ...
                await SendAsync<object>(HttpMethod.Put, $"/projects/{projectId}", payload);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> UpdateProjectMembersAsync(int projectId, List<ProjectMember> members)
        {
            try
            {
                ...
                var payloadMembers = members.Select(m => new
                {
                    userId = m.UserId,
                    role = string.IsNullOrWhiteSpace(m.Role) ? "member" : m.Role
                }).ToList();

                await SendAsync<object>(HttpMethod.Put, $"/projects/{projectId}/members", new { members = payloadMembers });
                return true;
            }
            catch (Exception ex)
            {
