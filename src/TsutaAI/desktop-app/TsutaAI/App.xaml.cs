...
namespace TsutaAI
{
    ...
    public partial class App : Application
    {
        ...
        /// <summary>
        /// 設定ファイルの API / Proxy 設定をランタイムサービスへ反映します。
        /// </summary>
        public static void ApplyNetworkSettings()
        {
            ...
            var proxySettings = settings?.Proxy ?? new ProxySettings();

            ApiService = new ApiService(apiSettings, proxySettings);
            Logger.Info($"ApiService を設定で再初期化しました。BaseUrl={apiSettings.BaseUrl}");

            bool wasConnected = WebSocketService?.IsConnected ?? false;
            if (WebSocketService != null)
            {
                try
                {
                ...
                {
                    Logger.Warn($"WebSocket切断中にエラーが発生しました: {ex.Message}");
                }
                finally
                {
                    WebSocketService.Dispose();
            ...
            WebSocketService = new WebSocketService(apiSettings.BaseUrl);
            if (Current is App app)
            {
                app.SetupWebSocketEventHandlers();
            }
            Logger.Info("WebSocketService を設定で再初期化しました。");

            if (wasConnected && CurrentUser != null && !string.IsNullOrWhiteSpace(CurrentUser.Token))
            {
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        Logger.Info("設定反映後に WebSocket を再接続しました。");
                    }
                    catch (Exception ex)
                    {
                        Logger.Warn($"設定反映後の WebSocket 再接続に失敗しました: {ex.Message}");
                    }
                });
            }
        }
        ...
        private void SetupWebSocketEventHandlers()
        {
            ...
            // 通知イベント
            WebSocketService.NotificationReceived += (sender, e) =>
            {
                Logger.Info("通知イベント受信: 通知バッジを即時更新します");
        ...
        private async Task RefreshNotificationBadgeAsync()
        {
            ...
            try
            {
                var unreadNotifications = await ApiService.GetNotificationsAsync(true);
                int unreadCount = unreadNotifications != null ? unreadNotifications.Count : 0;

                Dispatcher.Invoke(() =>
                {
                    foreach (Window window in Current.Windows)
                    {
                        var dashboardWindow = window as DashboardWindow;
                        ...
                        if (unreadCount > 0)
                        {
                            dashboardWindow.NotificationCountText.Text = unreadCount > 99 ? "99+" : unreadCount.ToString();
                            dashboardWindow.NotificationBadge.Visibility = Visibility.Visible;
                        }
                        else
                        {
                            dashboardWindow.NotificationCountText.Text = "0";
                            dashboardWindow.NotificationBadge.Visibility = Visibility.Collapsed;
                        }
                    }
                });
            }
            ...
            {
                Logger.Warn($"通知バッジ即時更新に失敗しました: {ex.Message}");
            }
        }
