...
async function createProject(req, res, next) {
  try {
    const payload = {
      ...
      createdBy: req.body.createdBy || req.body.created_by || 1,
...
async function updateProjectMembers(req, res, next) {
  try {
    ...
    if (!Array.isArray(userIds) && !Array.isArray(members)) {
      return res.status(400).json({ message: 'userIds または members は配列である必要があります。' });
    }

    const memberPayload = Array.isArray(members) ? members : userIds;
    await projectService.updateProjectMembers(projectId, memberPayload);
