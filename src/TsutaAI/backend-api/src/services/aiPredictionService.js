...
async function predictProjectRisk(context) {
    const { project, changes, historicalData, currentMetrics, historicalAlerts } = context;
 ...
 * 予測プロンプトを構築
 */
function buildPredictivePrompt({ project, changes, historicalData, currentMetrics, historicalAlerts, daysRemaining }) {
    const changesText = changes.map(c =>
        `- ${c.field}: ${c.oldValue} → ${c.newValue} (優先度: ${c.priority})`
    ).join('\n');
    const historicalAlertsText = formatHistoricalAlerts(historicalAlerts);
...
/**
 * 過去アラート履歴をプロンプト用テキストに変換
 */
function formatHistoricalAlerts(alerts) {
        ...
        .map((alert) => {
            ...
            if (alert.created_at) {
                ...
                if (!Number.isNaN(parsed.getTime())) {
                    createdAt = parsed.toISOString().split('T')[0];
                }
            }
            const status = alert.status || 'active';
            const severity = alert.severity || 'medium';
            const type = alert.type || 'warning';
            const summary = (alert.title || alert.message || '（内容なし）')
                .toString()
                .replace(/\s+/g, ' ')
                .slice(0, 80);

            return `- ${createdAt} [${status} | ${severity} | ${type}] ${summary}`;
        })
        .join('\n');
}
