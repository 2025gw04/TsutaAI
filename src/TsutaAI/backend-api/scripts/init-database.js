...
const BETTER_SQLITE3_BINARY_MISMATCH_HINTS = [
...
function isBetterSqlite3BinaryMismatch(error) {
  if (!error || !error.message) return false;
  const message = String(error.message);
  return message.includes('better_sqlite3.node') &&
    BETTER_SQLITE3_BINARY_MISMATCH_HINTS.some((hint) => message.includes(hint));
}

function printBetterSqlite3RecoverySteps() {
  console.error('\nERROR: better-sqlite3 のネイティブバイナリが現在の実行環境と一致していません。');
  console.error('原因: 別OSでインストールした node_modules を共有した可能性があります。');
  console.error('\n対処方法（backend-api ディレクトリで実行）:');
  if (process.platform === 'win32') {
    console.error('  1) npm rebuild better-sqlite3 --build-from-source');
  } else {
    console.error('  1) npm rebuild better-sqlite3');
  }
  console.error('  2) 改善しない場合は node_modules と package-lock.json を削除して npm install');
}

function preflightCheckNativeDependencies() {
  if (dbClient !== 'better-sqlite3') return;

  let BetterSqliteDatabase;
  try {
    // ネイティブモジュールのロード可否を先に確認し、DB削除前に失敗理由を明確化する
    BetterSqliteDatabase = require('better-sqlite3');
  } catch (error) {
    if (isBetterSqlite3BinaryMismatch(error)) {
      printBetterSqlite3RecoverySteps();
    }
    throw error;
  }
  ...
  let probeDb = null;
  try {
    // require だけではネイティブバインディングの不整合を検出できないケースがあるため、
    // 実際にインメモリDBを開いて最低限のクエリを実行する。
    probeDb = new BetterSqliteDatabase(':memory:');
    probeDb.prepare('SELECT 1 as ok').get();
  } catch (error) {
    if (isBetterSqlite3BinaryMismatch(error)) {
      printBetterSqlite3RecoverySteps();
    }
...
async function initDatabase() {
  ...
  try {
    preflightCheckNativeDependencies();
...
async function seedHelpRequests(knex, dbClient) {
  ...
  if (validTasks.length === 0) {
    console.log('    -> 有効なタスクがありません。ヘルプリクエストをスキップします。');
    return;
  }

  const now = new Date();
  const requests = [
    { projId: 7, title: '認証APIのJWT実装について', desc: 'Auth0との連携部分でトークン検証エラーが発生しています。', urgency: 'high', status: 'resolved', helper: 3, resolution: '公開鍵の取得エンドポイント設定が間違っていたため修正しました。', durationHours: 2 },
    { projId: 7, title: 'Svelteコンポーネントのライフサイクル', desc: 'onMountとonDestroyの挙動が想定と異なります。', urgency: 'medium', status: 'resolved', helper: 4, resolution: 'クリーンアップ関数の戻り値を修正し解決しました。', durationHours: 1 },
    { projId: 4, title: 'CRM連携のデータマッピング', desc: '旧システムと新システムの顧客IDマッピングで不整合が生じています。', urgency: 'critical', status: 'in_progress', helper: 11, durationHours: 0 },
    { projId: 6, title: 'NLPモデルの精度向上', desc: '特定の言い回しに対する意図認識率が低いです。', urgency: 'high', status: 'open', helper: null, durationHours: 0 },
    { projId: 4, title: 'UIデザインのアクセシビリティ', desc: 'コントラスト比がガイドラインを満たしていない箇所があります。', urgency: 'medium', status: 'resolved', helper: 6, resolution: 'カラーパレットを調整し解決。', durationHours: 4 },
    { projId: 7, title: 'RAGのベクトル検索精度向上', desc: '検索結果の適合率が低いため、リランキング処理を追加したいです。', urgency: 'high', status: 'open', helper: null, durationHours: 0 },
    { projId: 6, title: 'WebSocketの再接続ロジック', desc: 'モバイル環境での瞬断時にセッションが切れてしまいます。', urgency: 'medium', status: 'assigned', helper: 11, durationHours: 0 }
  ];

  for (const [idx, req] of requests.entries()) {
    const task = validTasks.find(t => t.project_id === req.projId) || validTasks[0];
    if (!task) continue;

    // 初期化実行日のデータとして見えるよう、当日内の時刻で作成
    const createdAt = new Date(now.getTime() - idx * 10 * 60 * 1000);
    let assignedAt = null;
    let assignedAtTime = null;
    let resolvedAt = null;

    if (req.helper) {
      const assignedAtCandidate = new Date(createdAt.getTime() + 1 * 60 * 60 * 1000);
      assignedAtTime = Math.min(assignedAtCandidate.getTime(), now.getTime());
      assignedAt = formatDateTimeLocal(new Date(assignedAtTime));
    }
    if (req.status === 'resolved') {
      const resolvedAtCandidate = new Date(createdAt.getTime() + (1 + req.durationHours) * 60 * 60 * 1000);
      const lowerBound = assignedAtTime ?? createdAt.getTime();
      const resolvedAtTime = Math.max(lowerBound, Math.min(resolvedAtCandidate.getTime(), now.getTime()));
      resolvedAt = formatDateTimeLocal(new Date(resolvedAtTime));
    }
    ...
    const helpRequestId = await getInsertedId(knex, dbClient, 'help_requests', {
      ...
      created_at: formatDateTimeLocal(createdAt)
    });
...
async function seedMentalHealthData(knex, dbClient) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const isExecutionDate = i === 0;

    // 実行日以外の土日はスキップ
    const day = reportDate.getDay();
    if (!isExecutionDate && (day === 0 || day === 6)) continue;

    for (const user of users) {
      // 実行日は必ずデータを作る。それ以外は約11%の確率で記録
      if (!isExecutionDate && Math.random() > 0.11) continue;
...
async function seedDevMatsumotoData(knex, dbClient) {
  ...
  try {
    ...
    // 1. 実行日のサンプル通知を2件追加
    const executionDateLabel = new Date().toLocaleDateString('ja-JP', {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit'
    });
    ...
    const now = new Date();
    const currentTimestamp = formatDateTimeLocal(now);

    const notifications = [
      {
        user_id: userId,
        title: `本日の作業通知（${executionDateLabel}）`,
        message: '本日の優先タスクを確認して、作業を開始してください。',
        type: 'info',
        is_read: 0,
      ...
      {
        user_id: userId,
        title: `ヘルプ対応リマインド（${executionDateLabel}）`,
        message: '未対応のヘルプリクエストがないか確認してください。',
        type: 'warning',
        is_read: 0,
        created_at: currentTimestamp
      }
