...
async function createTask(req, res, next) {
  try {
    ...
    // WebSocketでタスク作成を通知
    if (result && result.id) {
      websocketService.notifyTaskUpdate(result.id, payload.assigned_to, {
        action: 'created',
        task: result
      });
    }
...
async function updateTask(req, res, next) {
  try {
    ...
    if (updatedTask) {
      websocketService.notifyTaskUpdate(taskId, payload.assigned_to, {
        action: 'updated',
        task: updatedTask
      });
    }
...
async function deleteTask(req, res, next) {
  try {
    ...
    // WebSocketでタスク削除を通知
    if (task) {
      websocketService.notifyTaskUpdate(taskId, task.assigned_to, {
        action: 'deleted',
        taskId: taskId
      });
    }
