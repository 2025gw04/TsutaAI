...
function detectChanges(currentSnapshot, previousSnapshot) {
    ...
    // 遅延タスクの増加
    const currentOverdue = currentSnapshot.taskStats?.overdue || 0;
    const previousOverdue = previousSnapshot.taskStats?.overdue || 0;
    const overdueDiff = currentOverdue - previousOverdue;

    if (overdueDiff > 0) {
        changes.push({
            field: 'overdueTasks',
            priority: overdueDiff >= 3 ? 'high' : 'medium',
            oldValue: previousOverdue,
            newValue: currentOverdue,
            description: `遅延タスクが${overdueDiff}件増加しました`
        });
    }
